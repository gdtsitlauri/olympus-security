...
% Core row
\node[corebox, minimum width=3.5cm] (orch) {Orchestrator};
\node[corebox, minimum width=2.0cm, right=0.6cm of orch] (kb) {OLYMPUS-KB};
\node[corebox, minimum width=1.6cm, right=0.4cm of kb] (log) {Audit Log};

% Offensive modules
\node[redbox, below=0.7cm of orch, xshift=-2.1cm] (m1) {M1\\Pentest};
\node[redbox, right=0.25cm of m1] (m3) {M3\\Zero-Day};
\node[redbox, right=0.25cm of m3] (m9) {M9\\LLM Atk};

% Defensive modules
\node[bluebox, right=0.25cm of m9] (m2) {M2\\Virus};
\node[bluebox, right=0.25cm of m2] (m4) {M4\\Threat};
\node[bluebox, right=0.25cm of m4] (m5) {M5\\Decept};

% More modules row
\node[bluebox, below=0.35cm of m1, xshift=0.9cm] (m7) {M7\\SocEng};
\node[bluebox, right=0.25cm of m7] (m8) {M8\\AI-Int};
\node[bluebox, right=0.25cm of m8] (m10) {M10\\Forensic};
\node[corebox, right=0.25cm of m10] (m6) {M6\\TITAN};

% Arrows from orchestrator to module rows
\draw[arr] (orch.south) -- ++(0,-0.2) -| (m1.north);
\draw[arr] (orch.south) -- ++(0,-0.2) -| (m3.north);
\draw[arr] (orch.south) -- ++(0,-0.2) -| (m9.north);
\draw[arr] (orch.south) -- ++(0,-0.2) -| (m2.north);
\draw[arr] (orch.south) -- ++(0,-0.2) -| (m4.north);
\draw[arr] (orch.south) -- ++(0,-0.2) -| (m5.north);
\draw[arr] (orch.south) -- ++(0,-0.55) -| (m7.north);
\draw[arr] (orch.south) -- ++(0,-0.55) -| (m8.north);
\draw[arr] (orch.south) -- ++(0,-0.55) -| (m10.north);
\draw[arr] (orch.south) -- ++(0,-0.55) -| (m6.north);

% KB connections
\draw[arr, dashed, gray] (kb.south) -- ++(0,-0.15) -| (m4.north);
\draw[arr, dashed, gray] (kb.south) -- ++(0,-0.15) -| (m6.north);

% API/Dashboard below
\node[graybox, minimum width=1.8cm, below=0.5cm of m7, xshift=0.6cm] (api) {FastAPI :8000};
\node[graybox, minimum width=1.8cm, right=0.3cm of api] (dash) {Streamlit :8501};
\node[graybox, minimum width=1.4cm, right=0.3cm of dash] (gpu) {GTX 1650\\CUDA 12.x};

\draw[arr] (orch.south) -- ++(0,-1.1) -| (api.north);
\draw[arr] (orch.south) -- ++(0,-1.1) -| (dash.north);

\end{tikzpicture}
\caption{OLYMPUS system architecture. The orchestrator coordinates 10 modules
sharing OLYMPUS-KB. Red = offensive, blue = defensive, yellow = core/evolution.}
\label{fig:arch}
\end{figure}
...
\input{../results/comparison_tables/table_module2_malware_detection}
...
\input{../results/comparison_tables/table_module7_social_engineering}
...
\input{../results/comparison_tables/table_module9_llm_defense}
...
\input{../results/comparison_tables/table_module6_titan_evolution}
...
\input{../results/ablation/table_ablation}
