...
The current release uses lightweight seeded simulations to exercise the
benchmark/export pipeline for selected modules. These outputs are useful as
internal calibration artifacts and regression tests, but they are not a
substitute for real benchmark campaigns on external datasets.
"""
...
def run_all_experiments() -> None:
    ...
    experiment_results["module2_malware_detection"] = m2_results
    ...
    experiment_results["module7_social_engineering"] = m7_results
    ...
    experiment_results["module9_llm_defense"] = m9_results
    ...
    experiment_results["module6_titan_evolution"] = m6_results
    _print_table("TITAN vs Baselines", m6_results, ["best_fitness"])

    all_results = {
        "metadata": {
            "evidence_basis": "synthetic-calibration",
            "intended_use": "pipeline validation and internal comparative sanity checks",
            "not_for": "real-world benchmark claims, deployment performance claims, or external leaderboard comparisons",
            "seeds": SEEDS,
            "notes": [
                "Module 2, 7, and 9 benchmarks are seeded simulations, not external dataset evaluations.",
                "Module 6 compares simulated evolutionary convergence curves under a common synthetic scoring model.",
                "Use dedicated real datasets and task-specific benchmarks before making external performance claims.",
            ],
        },
    ...
    _generate_latex_tables(experiment_results)
...
def _write_results_readme() -> None:
    note_path = RESULTS_DIR.parent / "README.md"
    lines = [
        "# OLYMPUS Results Notes",
        "",
        "These result files are **synthetic calibration artifacts** generated by",
        "`experiments/baseline_comparison.py` and `experiments/ablation_study.py`.",
        "",
        "They are useful for:",
        "- exercising the export pipeline",
        "- checking metric formatting and reporting",
        "- comparing internal module variants under a shared toy setup",
        "",
        "They are **not** a substitute for:",
        "- external benchmark datasets",
        "- live traffic or operational security testing",
        "- real malware/phishing/jailbreak detection studies",
        "",
        "Before publishing strong empirical claims, rerun the affected modules on",
        "real datasets and persist the corresponding raw outputs alongside these",
        "synthetic calibration artifacts.",
        "",
    ]
